function RunAnalyzeData5Rig3(ExpDate,FlagServer)
...
DirNameSave=['C:\Data Analysis\Rig3\' ExpDate];

if FlagServer==1
    DirNameRead=['Y:\3D_RIG_3\2016\' ExpDate]; %server Z
elseif FlagServer==2
    DirNameRead=['K:\Microscope data\Rig3\' ExpDate]; %backup Hard drive
else
...
for jj=3:length(ExpTime)%1:length(ExpTime) %extracts the data for every stack
    ...
    if length(ExpTime(jj).name)>2 %remove "null files" that are erroneously found by matlab
        ...
        try
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Reads points traces
            if exist([DirNameRead_Time '\Functional_Data'],'dir') ~=0 %if points have been taken for this stack
                
                disp('I am now reading the points')
                
                [DataGreenCh,DataRedCh,Times]=extractdata2_rig3(DirNameRead_Time);
            ...
            %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
            % Reads speed data
            if exist([DirNameRead_Time '\Functional imaging TDMS data files\Speed_Data'],'dir') ~=0 %if speed data has been saved for this stack
                
                disp('I am now extracting the speed data')
                
                [Speed]=ExtractEncoderData_Rig3(DirNameRead_Time);
