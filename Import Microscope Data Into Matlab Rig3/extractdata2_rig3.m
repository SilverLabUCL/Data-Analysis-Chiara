...
function [DataGreenCh,DataRedCh,Times]=extractdata2_rig3(DirName)
addpath(genpath(DirName));
ReadFiles=dir([DirName '\Functional_Data']);
...
for i=1:length(ReadFiles)
        %Rig3 File Format: ROI-001_MSCN_Dwell-0.1us_ChA_Trial-01
        %Rig3 File Format: ROI-001_POI_Dwell-4.0us_ChA_Trial-01
        %Rig2 File Format: Dwel12_PMC_G1_ChA_1
        temp=sscanf(ReadFiles(i,1).name,'%*4c%u%*19c%c%*7c%u'); %test if it's a file with the data (i.e. it has a correct name)
...
ScanTime=0.0285; %time to move from one point to the next point, milliseconds
TrialStart=0;
