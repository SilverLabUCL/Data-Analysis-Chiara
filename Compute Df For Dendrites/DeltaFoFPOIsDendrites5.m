function [ DeltaFoverF_Sm, DeltaFoverF, TimesSegment, RedChannelData, DeltaFoverF_Sm_Lin , Times_Lin] = DeltaFoFPOIsDendrites5(PathExp,RedChFlag,SmoothingFactor)
...
if exist([PathExp '\pointTraces.mat']) == 2
...
%if only one mat file named "PutPoints..." is present in the folder, that file is loaded.
% Otherwise user selects mat file with POIsInDendrites information to load
MatFiles = dir('*.mat');  % find mat files
PutPointsFile = [];
counter = 0;

for i = 1:length(MatFiles)
    if strcmp(MatFiles(i,1).name(1:4),'PutP') == 1 % find mat files that start with PutP
        counter = counter + 1;
...
if RedChFlag
    [GreenOverRed]=CorrectMovementRedCh6(DataGreenCh,DataRedCh, Times, false);
...
for Seg=1:n_segments
    if isempty(PointsInSegments{1,Seg})==0 && nansum(MeanSegment(Seg,1,:)) ~=0 %if there are points in the segment
        ...
        SortedVal=SortedVal(SortedVal<HighLim);
        Baseline=nanmean(SortedVal);
%         LowLim=prctile(FluoSeg,5);
%         Baseline=nanmean(LowLim);
