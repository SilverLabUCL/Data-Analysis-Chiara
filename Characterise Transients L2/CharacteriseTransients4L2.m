function [ TransientsChar, ResponsesDistPerc, ResponsesDist, ResponsesBin ] = CharacteriseTransients4L2(SkipSegment, FlagPlot, FlagSave)
...
% compared to version for L5, spatial distribution computed differently:
% soma is ignored, and it accounts for higher levels of activity and more
% branches
...
Baseline =[33 36]; % start and end points, seconds
...
% donwsample data if too big
if length(DeltaFoverF_Sm_Lin) > 1E5
    
    disp('WARNING!! Downsampling data on time dimension')
    
    for seg = 1:size(DeltaFoverF_Sm_Lin,1)
        DeltaF(seg,:) = downsample(DeltaFoverF_Sm_Lin(seg,:),10);
        Tim(seg,:) = downsample(Times_Lin(seg,:),10);
    end
    
    DeltaFoverF_Sm_Lin = DeltaF;
    Times_Lin = Tim;
    TimeRes = mean(diff(Tim(1,:)));
else
...
for s = 1:length(Segments)
    ...
    for e = 1 : length( TransientsChar(1,Segments(s)).OnsetApprox )
        if isnan( TransientsChar(1,Segments(s)).OnsetApprox(e) ) == 0
...
%% measure spatial distribution for each response in dendrites. Not considering responses in the soma

[TransientsChar, ResponsesDistPerc, ResponsesDist] = SpatialDistributionSpikesL2(  ResponsesBin(2:end,:), Segments(2:end), TransientsChar, true );
