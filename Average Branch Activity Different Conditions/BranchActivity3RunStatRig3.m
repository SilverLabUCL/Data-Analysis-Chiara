function [RunningActivity, StationaryActivity] = BranchActivity3RunStatRig3(nStacks,n_segments,FlagSave, FilesLoaded)
%calculate branch activity for different animal states: running and stationary.

% compare to the code for rig2, this code takes into account different
% format of speed data

% NOTE: this code is going around 2 bugs:
% - First bug in the labview software, in speed data, times were not zeroed at
% the beginning of each trial, so here I am just taking the total length of
% the recording and dividing by number of trials
% - Second bug in the code that imports the data into matlab, fill time of
% AOD was set to 24 us instead of 24.5, so need to correct timescale of
% imaged POIs
%%%%% CAREFUL these 2 bugs have been fixed on the 27/4/2016, so if working
%%%%% on data acquired after this data, needs to correct the code
...
for s=1:nStacks
    ...
    load(FilesLoaded{s},'DeltaFoverF','Times','PointsInSegments','POIsIn2Segm','MeanSegment','NodesInfo','SortedTree')
    ...
    % correct time scale of imaged POIs
    [ ~, TimesSegment ] = CorrectTimeRig3( Times, PointsInSegments, POIsIn2Segm );
    
    % divide speed data in trials
    TrialLength = floor(size(Speed,1)/n_trials);
    
    SpeedTrials = NaN(n_trials, TrialLength);
    SpeedTime = NaN(n_trials, TrialLength);
    
    for t = 1:n_trials
        SpeedTrials(t,:) = Speed((t-1)*TrialLength+1 : t*TrialLength ,2);
        SpeedTime(t,:) = Speed((t-1)*TrialLength+1 : t*TrialLength ,1) - Speed( (t-1)*TrialLength+1,1);
    end
    ...
    for Seg=1:n_segments
        for t=1:n_trials
            ...
            SpeedInt=interp1(SpeedTime(t,:),SpeedTrials(t,:),TimesSegment(Seg,:));
    ...
    for Seg=1:n_segments
        ...
        BinSize=round(BinSizeMs/TimesSegment(Seg,2));
